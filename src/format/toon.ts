...
export const pluck = (key: string, subkey: string, as?: string): FieldDef => ({
  type: 'pluck',
  key,
  subkey,
  as,
});
export const custom = (as: string, fn: (i: Record<string, unknown>) => unknown): FieldDef => ({
  type: 'custom',
  as,
  fn,
});
...
function extract(item: Record<string, unknown>, schema: FieldDef[]): Record<string, unknown> {
  ...
  for (const def of schema) {
    ...
    if (def.type === 'field') out[key] = item[def.key] ?? null;
      ...
      throw new Error(`Unreachable: unknown FieldDef type ${(_exhaustive as FieldDef).type}`);
    }
  }
  return out;
}
