...
import { AsanaOAuthProvider, resolveClientCredentials } from '../auth/provider.js';
...
async function connect(): Promise<Client> {
  ...
  connecting = (async () => {
    if (!loadCredentials()?.tokens?.access_token) {
      throw new AxiError('Not authenticated with Asana', 'AUTH_REQUIRED', ['Run `asana-axi login`']);
    }
    // Access tokens are pre-registered per resolveClientCredentials(); the
    // StreamableHTTPClientTransport handles refresh via tokens()/saveTokens().
    const credentials = resolveClientCredentials();
    const provider = new AsanaOAuthProvider(() => {
      throw new AxiError('Asana session expired', 'AUTH_REQUIRED', ['Run `asana-axi login`']);
    }, credentials);
